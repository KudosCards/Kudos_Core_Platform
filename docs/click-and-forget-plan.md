...
Built, C1 to C8, and recorded in ADRs 0254 to 0261. The five questions this
started with are answered; what remains is at the end, and none of it is code.
...
- **C3 — Describe the catalog. Columns built (ADR 0259), the pass itself still
  to run.** Age band and tone on each birthday design, authored in Airtable
  beside the artwork so there is one author and a cleared cell correctly clears
  the column. Null is deliberately not "any": null is nobody having looked,
  "any" is somebody saying it suits everybody, and a selection rule has to tell
  silence from a claim. A word we cannot place is reported rather than
  swallowed. Describing the 217 designs is ops work —
  `docs/ops/catalog-describe-designs.md` — and until it is done the pick varies
  rather than matches.
...
- **C5 — Selection and messages. Built (ADR 0260), except the AI half.** The
  message pool had been saved and unread; this prints it. Where a message goes
  is settled with evidence rather than preference: the editor updates elements
  by id, so the seeded `inside-message` block survives every text edit and dies
  only when somebody deletes it — exactly when it stops being trustworthy. So
  that block, else the only text block on the inside page, else nothing, because
  two candidates is a coin toss printed on a card that cannot be recalled. A
  blank inside page is written on, since a member's own artwork starts blank and
  could otherwise never carry a message. Chosen at approval, applied at send to
  the card's own copy. The design pick reads C3's columns in tiers, preferring
  without ever excluding, and undescribed beats a mismatched band. The
  AI-assisted authoring is **not** built and is deliberately left out: the pool
  a customer writes by hand now demonstrably works, and drafting is a vendor and
  a data-processing agreement rather than a code change.
...
- **C8 — The messaging. Built (ADR 0261).** A homepage section and two FAQ
  answers, written under the rule the FAQ already had: every number from a
  constant, every claim something the product does today. The copy lives in
  `lib/click-and-forget.ts` rather than in JSX, so a test can hold it to that —
  the plan it names is the cheapest one carrying auto-send, the homepage and the
  FAQ name the same plan, the "you choose the cards and write the messages" line
  survives, and the two things we do not do stay unsaid. Shipped before a real
  standing order had posted a card, which was the owner's call against my advice
  twice given; the copy is written so that nothing in it depends on that having
  happened.
...
1. ~~**What does the selection rule do when it knows nothing?**~~ **Settled
   (ADR 0260).** A contact with no birth year — the CleanCloud default — falls
   through the tiers to a stable per-year hash of the pool, so the card varies
   between years and never repeats within one. Stated, not discovered.
2. **How many messages is a pool?** One is not personalisation; twenty is a
   ...
   rather than assume. Nothing enforces a count today: a pool of one is allowed
   and every card that year carries the same words.
