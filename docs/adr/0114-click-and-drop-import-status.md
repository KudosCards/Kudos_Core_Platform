...
Mail's stored `orderIdentifier`, any error, and the row's `updatedAt`. An
operator copies a sampled `ORD-…` reference straight into the Click & Drop
dashboard search to confirm our orders land in the same account as the legacy
`#NNNN` ones.
...
- No schema change: the readout is derived entirely from existing columns
  (`clickAndDropOrderId`, `clickAndDropError`, `updatedAt`).
...
- `updatedAt` is a proxy for "imported at" — there's no dedicated timestamp
  column — so a later unrelated update to a job would move its sample date. The
  bands themselves are exact; only the displayed time is approximate.
- Covered by e2e tests (imported band with a searchable reference, errored band
  with its message, ops-only auth).

## Alternatives considered

- **Add an `importedAt` column.** Rejected for now: a schema migration for a
  diagnostic panel isn't worth it while `updatedAt` is a good-enough proxy.
